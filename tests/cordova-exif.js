var CordovaExif = (function () {
  ...
  Exif = {
    find: function (image) {
      ...
      while (offset < image.length) {
        if (image.getByteAt(offset) === 0xFF) {
          ...
          if (image.getByteAt(offset + 1) === 0xE1) { return Exif.read(image, offset + 4) }
          offset += 2
        } else ++offset
      }
      return false
    ...
    readTags: function (image, exifStart, dirStart, strings, bigEndian) {
      ...
      var tags = {}
      var entryOffset
      var entries = image.getShortAt(dirStart, bigEndian)
  ...
  BinaryImage = function (imageBin) {
    ...
    this.getLongAt = function (imageOffset, binaryBigEndian) {
      var imageShort
      var imageByte1 = this.getByteAt(imageOffset)
      var imageByte2 = this.getByteAt(imageOffset + 1)
      var imageByte3 = this.getByteAt(imageOffset + 2)
      var imageByte4 = this.getByteAt(imageOffset + 3)
